...
- **로컬 완성판(2026-07-23)**: ① 워커 배치 커밋 — 드레인 150→**1,250건/s**(8.3배), 스트림 소비 통합 테스트 CI 편입 ② Alertmanager 실연동 — InstanceDown 발화→웹훅 수신 로그 검증(mock-notify /alert) ③ 훈련 5 패킷 유실 — WSL2에 netem 없음(`iptables -m statistic`으로 대체), 5% 유실 시 med 불변·p95 5→241ms(48배)·재전송 +47 (drill5 포스트모템) ④ 원커맨드: `.\scripts\start-local-ha.ps1` / `stop-local-ha.ps1 [-Infra]`
- **Stream vs Kafka 비교 완료(2026-07-28)**: `--coupon.record.mode=kafka` 추가(compose kafka 프로파일, KRaft 단일 노드). 성능 동급(드레인 1,000 vs 882건/s — DB 쓰기가 지배), 핵심 차이는 원자성 경계(Lua+XADD 원자 vs 판정-발행 비원자→outbox 필요)와 신규 그룹 시작점 함정의 대칭성(Stream latest ↔ Kafka auto.offset.reset). 현 규모 판단: Stream 유지. 리포트: docs/reports/phase5-stream-vs-kafka.md
- **잔여(실배포에서만 가능)**: AWS 배포 + 부하 생성기 분리 재측정, TIME_WAIT 정석 관찰, netem 정밀 재현, Kafka 파티션 병렬성·리밸런싱 관찰. **선행 조건: AWS 계정(결제 필요 — 보류 중)**. Phase 5 회고는 실배포 후 작성.
