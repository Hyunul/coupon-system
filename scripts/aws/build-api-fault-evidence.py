#!/usr/bin/env python3
"""Build a fail-closed, offline API-fault evidence manifest."""
...
INPUT_SCHEMAS = {
    "stop_receipt": {
        "run_id", "event_id", "target_instance_id", "action", "command_id", "status",
        "submitted_at", "completed_at", "service",
    },
    "start_receipt": {
        "run_id", "event_id", "target_instance_id", "action", "command_id", "status",
        "submitted_at", "completed_at", "service",
    ...
    "timeline": {
        "run_id", "event_id", "load_started_at", "stop_submitted_at", "stop_completed_at",
        "start_submitted_at", "start_completed_at", "recovery_completed_at",
    },
    "alb": {"run_id", "event_id", "status", "captured_at", "target_instance_id", "transitions"},
    "api": {
        "run_id", "event_id", "status", "captured_at", "successful_requests",
        "transport_failures", "transport_failure_limit", "unexpected_responses",
        "duplicate_issues", "over_issued",
    },
    "mysql": {
        "run_id", "event_id", "status", "captured_at", "over_issued", "duplicate_issues",
    },
}
TRANSITION_SCHEMA = {"state", "observed_at"}
ALB_STATES = {"healthy", "initial", "unhealthy", "draining", "unused", "unavailable"}
APPROVED_API_SERVICES = {"coupon-api-reactive"}


class EvidenceError(ValueError):
    """An input cannot support an API-fault claim."""
...
def require_exact_keys(payload, expected, name):
...
def read_input(name, raw_path, seen_identities):
    ...
    require_exact_keys(payload, INPUT_SCHEMAS[name], name)
...
def validate_inputs(inputs):
    ...
    run_id = nonempty_string(timeline["run_id"], "timeline.run_id")
    event_id = timeline["event_id"]
    if isinstance(event_id, bool) or not isinstance(event_id, int) or event_id <= 0:
        raise EvidenceError("timeline.event_id must be a positive integer")
    for name, payload in inputs.items():
        ...
        if isinstance(input_event_id, bool) or not isinstance(input_event_id, int) or input_event_id <= 0:
            raise EvidenceError("%s.event_id must be a positive integer" % name)
        if input_event_id != event_id:
    ...
    if stop_service != start_service:
        raise EvidenceError("stop and start receipts must identify the same API service")
    if stop_service not in APPROVED_API_SERVICES:
        raise EvidenceError("receipt service must be an approved API service identity")
    if stop_target != start_target:
        raise EvidenceError("receipt target instance IDs must match")
    ordered = (
        "load_started_at", "stop_submitted_at", "stop_completed_at", "start_submitted_at",
        "start_completed_at", "recovery_completed_at",
    )
    ...
    alb = inputs["alb"]
    alb_captured = validate_snapshot(alb, "alb")
    if nonempty_string(alb["target_instance_id"], "alb.target_instance_id") != stop_target:
        raise EvidenceError("alb.target_instance_id must match the selected API instance")
    transitions = alb["transitions"]
    if not isinstance(transitions, list) or len(transitions) < 3:
        raise EvidenceError("alb.transitions must contain healthy, away, and healthy observations")
    transition_times = []
    states = []
    for index, transition in enumerate(transitions):
        label = "alb.transitions[%d]" % index
        require_exact_keys(transition, TRANSITION_SCHEMA, label)
        state = nonempty_string(transition["state"], label + ".state")
        if state not in ALB_STATES:
            raise EvidenceError("%s.state is not an ALB target state" % label)
        states.append(state)
        transition_times.append(utc_timestamp(transition["observed_at"], label + ".observed_at"))
    if any(earlier > later for earlier, later in zip(transition_times, transition_times[1:])):
        raise EvidenceError("alb transition timestamps are not ordered")
    away_times = [observed_at for state, observed_at in zip(states[1:-1], transition_times[1:-1]) if state != "healthy"]
    if states[0] != "healthy" or states[-1] != "healthy" or not away_times:
        raise EvidenceError("alb transitions must move away from healthy and recover to healthy")
    if transition_times[0] < moments["load_started_at"] or transition_times[-1] > moments["recovery_completed_at"]:
        raise EvidenceError("alb transitions are outside the active load/fault/recovery timeline")
    if transition_times[0] > stop_completed or transition_times[-1] < start_completed or not any(
        stop_completed <= observed_at <= start_completed for observed_at in away_times
    ):
        raise EvidenceError("alb transitions do not cover the API fault window")
    if alb_captured <= transition_times[-1] or alb_captured < moments["recovery_completed_at"]:
        raise EvidenceError("alb snapshot must follow recovered health and recovery evidence")
    ...
    for field in ("transport_failures", "transport_failure_limit", "unexpected_responses", "duplicate_issues", "over_issued"):
        nonnegative_integer(api[field], "api." + field)
    if api["transport_failures"] > api["transport_failure_limit"]:
        raise EvidenceError("api.transport_failures exceeds transport_failure_limit")
    if api["unexpected_responses"] != 0 or api["duplicate_issues"] != 0 or api["over_issued"] != 0:
        raise EvidenceError("api reconciliation failed")
    if api_captured < moments["recovery_completed_at"]:
        raise EvidenceError("api snapshot predates recovery")
    ...
    if mysql_captured < moments["recovery_completed_at"]:
        raise EvidenceError("mysql snapshot predates recovery")
...
def main(argv=None):
    ...
    try:
        ...
        parser.exit(1, "build-api-fault-evidence: %s\n" % error)
