...
## 5. 후속 실증 (2026-07-23 추가) — "Netty가 이기는 조건"의 확인

같은 의미론(응답 전 알림 완료 대기, 3s 지연 주입)·같은 100rps에서 스레드 블록 대기와 논블로킹 대기를 직접 비교했다 (`k6-results/nw-*.json`):

| | servlet (Tomcat 스레드가 3s 블록) | reactive (이벤트 루프 논블로킹 대기) |
|---|---|---|
| 완료 | 9,000건 + dropped 2,166 | **12,001건 완주, dropped 0** |
| med / p95 | **22,508ms / 38,504ms** (대기열 발산) | **3,276ms / 3,277ms** (고유 지연 3s + ε) |

용량 산수(200÷3s≈67rps < 100rps)가 그대로 재현됐고, 논블로킹 버전은 고유 지연만 남기고 100rps를 완주했다. 4장의 "역전 없음"과 합치면 결론이 완성된다: **스레드가 외부 I/O에 묶이는 워크로드에서만 Netty가 이긴다. 임계 경로가 이미 논블로킹에 가깝다면 Tomcat도 충분하다.**
